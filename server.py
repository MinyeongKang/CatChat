...
def messageDB(id, message, ip, port, date):
    print("Message Insert about message .... ")
    try:
        ...
        sql_insert_data_query = """ INSERT INTO cat_talk_chatting(user_id, message, ip_address, port_number, time) VALUES(%s,%s,%s,%s,%s) """

        insert_message_data = (id, message, ip, port, date)

        result = cursor.execute(sql_insert_data_query, insert_message_data)

        connection.commit()
        print("message data inserted successfully into cat_talk_chatting table", result)

    except mysql.connector.Error as error:
        print("Failed inserting message data MySQL table {}".format(error))
        ...
        while self.userOnline and self.server.chat:
            try:
                ...
                if msg_length:
                    ...
                    print(self.dbmsg)
                    print("테스트중입니다...")
                    messageDB(self.username, self.dbmsg, self.addr[0], self.addr[1], datego)
                    # 메세지 처리를 위한 함수 호출
                    self.handleMsg(msg)
            except:
                # 오류 또는 연결 실패가 있는 경우
